...
void sort_by_golch(Student a[], int n){
	...
	for(j = 0; j < n-i-1; j++){
		if(a[j].golch > a[j+1].golch){
			Student st;
			strcpy(st.fname, a[j].fname);
			strcpy(st.lname, a[j].lname);
			strcpy(st.id, a[j].id);
			st.golch = a[j].golch;
			strcpy(a[j].fname, a[j+1].fname);
			strcpy(a[j].lname, a[j+1].lname);
			strcpy(a[j].id, a[j+1].id);
			a[j].golch = a[j+1].golch;
			strcpy(a[j+1].fname, st.fname);
			strcpy(a[j+1].lname, st.lname);
			strcpy(a[j+1].id, st.id);
			a[j+1].golch = st.golch;
